jQuery(document).ready(function($) {
    ...
    function handleRemoveGroup() {
        ...
        $groupRow.find('.beaver-builder-class-dropdown-classes tbody tr').each(function() {
            ...
            if ($classInputs.filter(function() {
                    return $(this).val();
                }).length > 0) {
    ...
    function checkEarlyReturn() {
        ...
        groupsNames.forEach(el => {
            ...
            if (!el.value) {
                alert('One or more Group Names have no title. Please correct before submitting.');
                earlyReturn = true;
            }
        });
        ...
        classRows.forEach((el) => {
            ...
            if (!value.value || !label.value) {
                alert('One or more of the added classes have an empty class or label. Please correct before submitting.');
                earlyReturn = true;
            }
            ...
            collection = {
                ...
                ...{
                    [groupname.value]: {
                        ...
                        singleton: (singleton.checked ? '1' : '0')
                    }
                }
            };
        });
    ...
    $('#reset-bb-dropdown-settings').on('click', function(e) {
        e.preventDefault();
        var currentURL = new URL(window.location.href);
        currentURL.searchParams.set('clear_bb_class_options', '1');

        if (confirm('Are you sure you want to delete all classes?')) {
            window.location.href = currentURL.toString();
        }
    });
    // After the page reloads, remove the param so it's clean and the user can refresh the page
    var currentURL = window.location.href;
    var clearBBClassParam = 'clear_bb_class_options=1';
    if (currentURL.indexOf(clearBBClassParam) !== -1) {
        var updatedURL = currentURL.replace(new RegExp('&?' + clearBBClassParam), '');
